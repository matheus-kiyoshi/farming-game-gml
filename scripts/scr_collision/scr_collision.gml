function scr_collision() {
    sprite_bbox_top = (sprite_get_bbox_top(sprite_index)-sprite_get_yoffset(sprite_index))*image_yscale;
    sprite_bbox_bottom = (sprite_get_bbox_bottom(sprite_index)-sprite_get_yoffset(sprite_index)+1)*image_yscale;
    sprite_bbox_left = (sprite_get_bbox_left(sprite_index)-sprite_get_xoffset(sprite_index))*image_xscale;
    sprite_bbox_right = (sprite_get_bbox_right(sprite_index)-sprite_get_xoffset(sprite_index)+1)*image_xscale;
    ...
    var slope_num = collision_real_list(0,0,obj_Slope,slope_list);
    if slope_num > 0 {
        var on_slope;
    ...
    //Sidewall collisions
    #region //Horizontal
    var slope_x = collision_real_id(hspd,0,obj_Slope);
    if slope_x != noone {
        ...
        if !slope_x.connected {
            ...
            || (x > slope_x.x1 && slope_xscale < 0) {
                if slope_x.image_yscale > 0 {
                    if bbox_bottom > slope_x.y1 wall_collision(obj_Slope);
                } else {
                    if bbox_top < slope_x.y1 wall_collision(obj_Slope);
                }
            }
        }
    }
    #endregion
    #region //Vertical
    var slope_y = collision_real_id(0,vspd,obj_Slope);
    if slope_y != noone {
        ...
        if !slope_y.connected {
            ...
            || (y > slope_y.y2 && slope_yscale < 0) {
                if slope_y.image_xscale > 0 {//Normal
                    if bbox_right > slope_y.x2 wall_collision(obj_Slope);
                } else {//Inverted
                    if bbox_left < slope_y.x2 wall_collision(obj_Slope);
                }
            }
        }
    }
...
function snap_to_slope(slope_obj) {//Slope snap function
    ...
    if slope_obj.image_xscale > 0 {//Normal
        ...
        if hspd > 0 {
            if collision_real(0,-sign(slope_obj.image_yscale)/100,obj_Collision) {
                hspd = 0;
                snap = false;
            }
    ...
    if slope_obj.image_yscale > 0 {//Normal
        ...
        if vspd > 0 {
            if collision_real(-sign(slope_obj.image_xscale)/100,0,obj_Collision) {
                vspd = 0;
                snap = false;
            }
